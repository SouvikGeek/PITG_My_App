...
def create_app():
    ...
    with app.app_context(): #creating an app context
        ...
        def send_mail():
            ...
            image_url = 'https://processitglobal.com/ariUploads/Email-Header.png'


            # Create the email message with HTML formatting
            message_body = f"<p><img src='{image_url}' alt='Company Logo' width='400' height='120'></p>"
        ...
        #route for deactivation of employee
        @app.route('/enable_employee', methods=['GET', 'POST'])
        @login_required
        def enable_employee():
            error_message = None
            users = []
            ...
            if request.method == 'POST':
                search_filter = request.form.get('searchFilter')
                users = search_deactive_user(search_filter)
                ...
                if 'activate' in request.form:
                    ...
                    print('saved to the db')
                    activate_mail = activation_mail('pending',str(requested_id), str(session.get('username')), str(get_management))

                    return redirect('/view_all_requests')

            return render_template('activate_employee.html', users=users, error_message=error_message)
                ...
                if task:
                    ...
                    if task.tat_task_state == 'pending':
                        ...
                        if tat_task_type=='deactivation':
                            ...
                            #sending mail
                            deactivation_approve_mail = deactivation_mail('approved', str(task.tat_request_by), str(task.tat_requested_id), str(get_management))
                            ...
                            #sending mail
                            atr_approve_mail = atr_mail('approved', str(task.tat_request_by), str(task.tat_requested_id), str(get_management))   
                            
                        elif tat_task_type=='activation':
                            isactivated = modify_ldap_entry_activation(str(tat_effected_id)) 
                            #sending mail                        
                            activation_approve_mail = activation_mail('approved', str(task.tat_request_by), str(task.tat_requested_id), str(get_management)) 
            ...
            if request.method == "POST":
                ...
                if task:
                    ...
                    if task.tat_task_state == 'pending':
                        ...
                        if task.tat_task_type == 'deactivation':
                            deactivation_reject_mail = deactivation_mail('rejected', str(task.tat_request_by), str(task.tat_requested_id), str(get_management))
                        elif task.tat_task_type == 'activation':
                            activation_reject_mail = activation_mail('rejected', str(task.tat_request_by), str(task.tat_requested_id), str(get_management))
                        elif task.tat_task_type == 'access_to_resource':
                            atr_reject_mail = atr_mail('rejected', str(task.tat_request_by), str(task.tat_requested_id), str(get_management))
        ...
        def access_to_resource():
            ...
            if request.method == 'POST':
                ...
                if 'access' in request.form:
                    ...
                    access_mail = atr_mail('pending', str(session.get('username')),  str(get_manager), str(reason_for_access))
        ...
        def test2():
            f= update_address_or_phone('cn=rrosha,ou=users,o=pitg', '1111122222', 'Delhi')
            return "OOps"
